...
const FeaturedJobs = () => {
  ...
  const skeletonCard = (i: number) => (
    <div key={i} className="bg-card border border-border rounded-xl p-5 animate-pulse shrink-0 w-[260px] sm:w-auto">
    ...
    <>
      <section className="py-12 md:py-24 bg-background">
        <div className="max-w-[1280px] mx-auto">
          <div className="flex items-end justify-between mb-8 md:mb-12 px-4">
          ...
          {loading ? (
            <div className="flex gap-4 overflow-x-auto px-4 pb-2 scrollbar-none md:grid md:grid-cols-4 md:overflow-visible md:pb-0">
              {[...Array(8)].map((_, i) => skeletonCard(i))}
            </div>
          ) : jobs.length === 0 ? (
            <p className="text-center text-muted-foreground py-12 px-4">No jobs available</p>
          ) : (
            <div className="flex gap-4 overflow-x-auto px-4 pb-3 scrollbar-none md:grid md:grid-cols-4 md:gap-6 md:overflow-visible md:pb-0">
              {jobs.map((job) => (
                <div
                  key={job.id}
                  className="bg-card border border-border rounded-xl p-5 hover:shadow-lg hover:border-primary/30 transition-all duration-200 shrink-0 w-[260px] sm:w-auto md:w-auto"
                >
                  <div className="flex items-center justify-between mb-4">
                  ...
                  <h3 className="font-semibold text-foreground mb-1 text-sm">{job.title}</h3>
                  ...
                  <div className="flex items-end justify-between gap-2">
                    ...
                    <Button size="sm" className="shrink-0 min-h-[36px]" onClick={() => setApplyingJob(job)}>
                      Apply
                    </Button>
                  </div>
                </div>
